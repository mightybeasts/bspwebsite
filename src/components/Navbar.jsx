    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 transition-colors duration-300 ease-in-out ${scrolled ? "bg-black" : "bg-transparent"}`}>
      <div className="w-full flex justify-between items-center max-w-7x1 mx-auto ">
        ...
        >
          ...
          <p className="text-white text-[18px] font-bold cursor-pointer flex">BLACK SPADES 
        ...
        <ul className='list-none hidden sm:flex flex-row gap-10'>
          {navLinks.map((nav)=>(
            <li key={nav.id}
              className={`${active === nav.title ? "text-slate-600" : "text-white"}
              hover:text-slate-600  text-[18px] font-medium cursor-pointer `}
